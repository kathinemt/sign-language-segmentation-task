...
def total_iou(gold_labels: np.ndarray, pred_labels: np.ndarray) -> float:
    """
    Used scikit's Jaccard score to account for IoU. Labels B, I chosen to be "inside".
    Micro-average used to account for "global" calculation for IoU in the paper.
    """
    return jaccard_score(gold_labels, pred_labels, labels=[B, I], average='micro', zero_division=0)
